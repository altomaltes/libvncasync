...
static void clientgone( rfbClient * cl )
{ if ( cl )
  { FREE( cl->clientData );
  }
}

static enum rfbNewClientAction newclient( rfbClient * cl )
...
static void doptr(int buttonMask,int x,int y,rfbClient * cl)
...
static void dokey(rfbBool down,rfbKeySym key,rfbClient * cl)
{ if(down)
  { if(key==XK_Escape)
      rfbCloseClient(cl);
    else if(key==XK_F12)
