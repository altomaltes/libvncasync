...
static rfbBool SendRectEncodingTight(rfbClient * cl, int x, int y,
                                     int w, int h);
static void FindBestSolidArea (rfbClient * cl, int x, int y, int w, int h,
                               uint32_t colorValue, int *w_ptr, int *h_ptr);
static void ExtendSolidArea   (rfbClient * cl, int x, int y, int w, int h,
                               uint32_t colorValue,
                               int *x_ptr, int *y_ptr, int *w_ptr, int *h_ptr);
static rfbBool CheckSolidTile    (rfbClient * cl, int x, int y, int w, int h,
                                  uint32_t *colorPtr, rfbBool needSameColor);
static rfbBool CheckSolidTile8   (rfbClient * cl, int x, int y, int w, int h,
                                  uint32_t *colorPtr, rfbBool needSameColor);
static rfbBool CheckSolidTile16(rfbClient * cl, int x, int y, int w, int h, uint32_t *colorPtr, rfbBool needSameColor);
static rfbBool CheckSolidTile32  (rfbClient *, int x, int y, int w, int h, uint32_t *colorPtr, rfbBool needSameColor);

static rfbBool SendRectSimple    (rfbClient *, int x, int y, int w, int h);
static rfbBool SendSubrect       (rfbClient *, int x, int y, int w, int h);

static rfbBool SendSolidRect     (rfbClient *);
static rfbBool SendMonoRect      (rfbClient *, int x, int y, int w, int h);
static rfbBool SendIndexedRect   (rfbClient *, int x, int y, int w, int h);
static rfbBool SendFullColorRect (rfbClient *, int x, int y, int w, int h);

static rfbBool CompressData (rfbClient * cl, int streamId, int dataLen,
                             int zlibLevel, int zlibStrategy);

static void FillPalette8 (int count);
static void FillPalette16 (int count);
static void FillPalette32 (int count);
static void FastFillPalette16 (rfbClient * cl, uint16_t *data, int w,
                               int pitch, int h);
static void FastFillPalette32 (rfbClient * cl, uint32_t *data, int w,
                               int pitch, int h);

static void PaletteReset (void);
static int PaletteInsert (uint32_t rgb, int numPixels, int bpp);

static void Pack24 (rfbClient * cl, char *buf, rfbPixelFormat *fmt,
                    int count);
...
static rfbBool SendJpegRect (   rfbClient *, int x, int y, int w, int h, int quality );
static void PrepareRowForImg(   rfbClient *, uint8_t *dst, int x, int y, int count);
static void PrepareRowForImg24( rfbClient *, uint8_t *dst, int x, int y, int count);
static void PrepareRowForImg16( rfbClient *, uint8_t *dst, int x, int y, int count);
static void PrepareRowForImg32( rfbClient *, uint8_t *dst, int x, int y, int count);

#ifdef HAVE_LIBPNG
static rfbBool SendPngRect(rfbClient * cl, int x, int y, int w, int h);
static rfbBool CanSendPngRect(rfbClient * cl, int w, int h);
#endif

/**
 *  Tight encoding implementation.
 */
int rfbNumCodedRectsTight( rfbClient * cl
                         , int x, int y
                         , int w, int h )
{ int maxRectSize, maxRectWidth;
...
rfbBool rfbSendRectEncodingTight( rfbClient * cl
                                , int x, int y
                                , int w, int h )
...
rfbBool rfbSendRectEncodingTightPng( rfbClient * cl
                                   , int x, int y
                                   , int w, int h )
...
rfbBool SendRectEncodingTight( rfbClient * cl
                             , int x, int y
                             , int w, int h )
{ int nMaxRows;
  uint32_t colorValue;
...
static void FindBestSolidArea( rfbClient * cl
                             , int x, int y
                             , int w, int h
                             , uint32_t colorValue
                             , int *w_ptr,  int *h_ptr)
...
static void ExtendSolidArea( rfbClient * cl
                             , int x, int y
                             , int w, int h
                             , uint32_t colorValue
                             , int * x_ptr, int * y_ptr
 ...
 */
static rfbBool CheckSolidTile( rfbClient * cl
                             , int x, int y
                             , int w, int h
                             , uint32_t* colorPtr
                             , rfbBool needSameColor )
{ switch(cl->screen->window.serverFormat.bitsPerPixel)
...
#define DEFINE_CHECK_SOLID_FUNCTION( bpp )                                    \
                                                                              \
static rfbBool                                                                \
CheckSolidTile##bpp( rfbClient * cl, int x, int y, int w, int h,              \
    uint32_t* colorPtr, rfbBool needSameColor)                                  \
{                                                                             \
    uint##bpp##_t *fbptr;                                                     \
...
static rfbBool CheckSolidTile32( rfbClient * cl
                               , int x, int y
                               , int w, int h
                               ,  uint32_t* colorPtr
                               , rfbBool needSameColor )
...
static rfbBool SendRectSimple( rfbClient * cl
                               , int x, int y
                               , int w, int h )
...
static rfbBool SendSubrect( rfbClient * cl
                            , int x, int y
                            , int w, int h )
{ char * fbptr;
  ...
  if ( cl->format.bitsPerPixel    == cl->screen->window.serverFormat.bitsPerPixel
       && cl->format.redMax       == cl->screen->window.serverFormat.redMax
       && cl->format.greenMax     == cl->screen->window.serverFormat.greenMax
       && cl->format.blueMax      == cl->screen->window.serverFormat.blueMax
  ...
  { (*cl->translateFn)(cl->translateLookupTable, &cl->screen->window.serverFormat,
    ...
    { case  8:        FillPalette8( w * h);        break;
      case 16:        FillPalette16(w * h);        break;
      default:        FillPalette32(w * h);
    }
  }
...
rfbBool rfbSendTightHeader( rfbClient * cl
                            , int x, int y
                            , int w, int h)
...
static rfbBool SendSolidRect( rfbClient * cl )
{ int len;
...
static rfbBool SendMonoRect( rfbClient * cl
                             , int x, int y
                             , int w, int h )
{ int streamId = 1;
  ...
  switch (cl->format.bitsPerPixel) /** Prepare palette, convert image. */
  { case 32:
...
static rfbBool SendIndexedRect( rfbClient * cl
                                , int x, int y
                                , int w, int h )
{ int streamId = 2;
  int i, entryLen;
  ...
  return CompressData( cl, streamId
                       , w * h, tightConf[compressLevel].idxZlibLevel
                       , Z_DEFAULT_STRATEGY );
}

static rfbBool SendFullColorRect( rfbClient * cl
                                  , int x, int y
                                  , int w, int h )
{ int streamId = 0;
  int len;
...
static rfbBool CompressData( rfbClient * cl
                             , int streamId
                             , int dataLen
                             , int zlibLevel, int zlibStrategy )
{ z_streamp pz;
...
rfbBool rfbSendCompressedDataTight( rfbClient * cl, char *buf,
                                    int compressedLen)
{ int i, portionLen;
...
#define DEFINE_FAST_FILL_PALETTE_FUNCTION(bpp)                          \
                                                                        \
static void                                                             \
FastFillPalette##bpp(rfbClient * cl, uint##bpp##_t *data, int w,       \
                     int pitch, int h)                                  \
{                                                                       \
    ...
    if (cl->translateFn != rfbTranslateNone) {                          \
        mask = cl->screen->window.serverFormat.redMax                          \
            << cl->screen->window.serverFormat.redShift;                       \
        mask |= cl->screen->window.serverFormat.greenMax                       \
             << cl->screen->window.serverFormat.greenShift;                    \
        mask |= cl->screen->window.serverFormat.blueMax                        \
             << cl->screen->window.serverFormat.blueShift;                     \
    } else mask = ~0;                                                   \
                                                                        \
    c0 = data[0] & mask;                                                \
    for (j = 0; j < h; j++) {                                           \
...
static void Pack24(rfbClient * cl,
                   char *buf,
                   rfbPixelFormat *fmt,
                   int count)
{ uint32_t *buf32;
  ...
  if (!cl->screen->window.serverFormat.bigEndian == !fmt->bigEndian)
...
static rfbBool
SendJpegRect(rfbClient * cl, int x, int y, int w, int h, int quality)
{ unsigned char *srcbuf;
  int ps = cl->screen->window.serverFormat.bitsPerPixel / 8;
  ...
  { uint16_t *srcptr, pix;
    ...
    { uint16_t *srcptr2 = srcptr;
      ...
      for (i = 0; i < w; i++)
      { pix = *srcptr2++;
        inRed   = (int) (pix >> cl->screen->window.serverFormat.redShift   & cl->screen->window.serverFormat.redMax);
        inGreen = (int) (pix >> cl->screen->window.serverFormat.greenShift & cl->screen->window.serverFormat.greenMax);
        inBlue  = (int) (pix >> cl->screen->window.serverFormat.blueShift  & cl->screen->window.serverFormat.blueMax);

        *dst2++ = (uint8_t)((inRed   * 255 + cl->screen->window.serverFormat.redMax   / 2) / cl->screen->window.serverFormat.redMax );
  ...
  { if (cl->screen->window.serverFormat.bigEndian && ps == 4)
    { flags |= TJ_ALPHAFIRST; }
    if (cl->screen->window.serverFormat.redShift == 16
        && cl->screen->window.serverFormat.blueShift == 0)
    { flags |= TJ_BGR; }
...
static void
PrepareRowForImg(rfbClient * cl,
                 uint8_t *dst,
                 int x,
                 int y,
                 int count)
{ if (cl->screen->window.serverFormat.bitsPerPixel == 32)
  { if ( cl->screen->window.serverFormat.redMax == 0xFF &&
         cl->screen->window.serverFormat.greenMax == 0xFF &&
         cl->screen->window.serverFormat.blueMax == 0xFF )
...
static void PrepareRowForImg24( rfbClient * cl
                                , uint8_t *dst
...
{ uint32_t *fbptr;
  ...
  while (count--)
  { pix = *fbptr++;
    *dst++ = (uint8_t)(pix >> cl->screen->window.serverFormat.redShift);
    *dst++ = (uint8_t)(pix >> cl->screen->window.serverFormat.greenShift);
    *dst++ = (uint8_t)(pix >> cl->screen->window.serverFormat.blueShift);
} }

#define DEFINE_JPEG_GET_ROW_FUNCTION(bpp)                                   \
                                                                            \
static void                                                                 \
PrepareRowForImg##bpp(rfbClient * cl, uint8_t *dst, int x, int y, int count) { \
    uint##bpp##_t *fbptr;                                                   \
    uint##bpp##_t pix;                                                      \
    ...
    while (count--) {                                                       \
        pix = *fbptr++;                                                     \
                                                                            \
        inRed = (int)                                                       \
            (pix >> cl->screen->window.serverFormat.redShift   & cl->screen->window.serverFormat.redMax); \
        inGreen = (int)                                                     \
            (pix >> cl->screen->window.serverFormat.greenShift & cl->screen->window.serverFormat.greenMax); \
        inBlue  = (int)                                                     \
            (pix >> cl->screen->window.serverFormat.blueShift  & cl->screen->window.serverFormat.blueMax); \
                                                                            \
  *dst++ = (uint8_t)((inRed   * 255 + cl->screen->window.serverFormat.redMax / 2) / \
                         cl->screen->window.serverFormat.redMax);                  \
  *dst++ = (uint8_t)((inGreen * 255 + cl->screen->window.serverFormat.greenMax / 2) / \
                         cl->screen->window.serverFormat.greenMax);                \
  *dst++ = (uint8_t)((inBlue  * 255 + cl->screen->window.serverFormat.blueMax / 2) / \
                         cl->screen->window.serverFormat.blueMax);                 \
    }                                                                       \
}
...
static void pngWriteData( png_structp png_ptr
                          , png_bytep   data
                          , png_size_t  length )
{
#if 0
  rfbClient * cl = png_get_io_ptr(png_ptr);
