...
#define MAX_SECURITY_TYPES 255
  ...
  switch (primaryType)
  { case rfbSecTypeNone   : rfbRegisterSecurityHandler(&VncSecurityHandlerNone   ); break;
    case rfbSecTypeVncAuth: rfbRegisterSecurityHandler(&VncSecurityHandlerVncAuth); break;
  }
...
{ int32_t securityType = rfbSecTypeInvalid;
  ...
  if (cl->protocolMajorVersion==3 && cl->protocolMinorVersion < 7) /* Make sure we use only RFB 3.3 compatible security types. */
  ...
  { rfbSendSecurityTypeList(cl, securityType); /* Here it's ok when securityType is set to rfbSecTypeInvalid. */
  }
}
...
{ rfbSecurityHandler* handler;
  ...
  unsigned char * chosenType= getStreamBytes( cl, 1 );
...
{ // int n;
  uint32_t authResult;
  uint8_t * response= (uint8_t *)getStreamBytes( cl, CHALLENGESIZE );
  if ( !response ) { return; }
